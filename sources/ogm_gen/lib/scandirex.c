...
PUBLIC(int) OgScanDirEx(char *path0, int (*func) (void *, int, int, char *, struct og_stat *), void *ptr, int mask, char *logfile)
{
int retour;
...
if (stat(path0,&filestat)==0 && !OgIsDir(filestat.st_mode)) {
  if (OgStat(path0,(mask|DOgStatMask_is_dir),&og_filestat)==0 && 
      og_filestat.is_dir) is_dir=1; else is_dir=0;
  IFE((*func)(ptr,0,strlen(path0),path0,&og_filestat));
  DONE;
  }
...
while(1) {
  ...
  do {
    ...
    #if (DPcSystem == DPcSystemUnix)
    if (OgStat(path,(mask|DOgStatMask_is_dir),&og_filestat)==0 && 
        og_filestat.is_dir) is_dir=1; else is_dir=0;
    #else
    /** This is 6 times faster than OgStat **/
    IFE(OgStatForDir(str_file,(mask|DOgStatMask_is_dir),&og_filestat));
    if (og_filestat.is_dir) is_dir=1; else is_dir=0;
    #endif
...
/******************************************************************/
   ...
   if (S_ISLNK(linkstat.st_mode)) is_lnk = 1; else is_lnk = 0;
    ...
    IFE((*func)(ptr,is_dir,ipath,path,&og_filestat));
    if (is_dir && !is_lnk) {
