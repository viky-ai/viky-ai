/*
 *	This function changes HTML encoded string into Latin encoded strin
...
static int OgHtmlToPagecode1(int, unsigned char *, int *);
static int OgHtmlToPagecode2(unsigned char *, int, 
                               unsigned char *, int *, int, int);



struct html_to_unicode {
  unsigned char *tag;
  int len,code;
  };

static struct html_to_unicode HtmlToUnicode[] = {
  ...
  {"atilde" ,6 ,227}, /* latin small letter a with tilde, U+00E3 ISOlat1 */
  ...
  {"ouml"   ,4 ,246}, /* latin small letter o with diaeresis, U+00F6 ISOlat1 */
...
/* there is no Sigmaf, and no U+03A2 character either */
  ...
  {"omega"    ,5 ,969}, /* greek small letter omega, U+03C9 ISOgrk3 */
...
/* prod is NOT the same character as U+03A0 'greek capital letter pi' 
  ...
  {"sum"      ,3 ,8721}, /* n-ary sumation, U+2211 ISOamsb */
...
/* black here seems to mean filled as opposed to hollow */
  ...
  {"hearts"   ,6 ,9829}, /* black heart suit = valentine, U+2665 ISOpub */
...
/*
 *  Transforme une chaîne de caractère HTML en une chaine
 *  dans une page de code dépendant de flags. Si flag vaut 1
 *  la chaine 'output' se code sur un seul octet et les caractères 
 *  dépassant 255 ne sont pas décodés, si flag vaut 2, les caractères
 *  dépassant 127 ne sont pas décodés. Typiquement on traite:
 *    &s; avec s dans la liste HtmlToUnicode 
 *    &#N; avec N ayant 1 à 4 digits
*/

PUBLIC(int) OgHtmlToPagecode(unsigned char *in, unsigned char *out, int *iout, int flag)
{
int i,j,k,code,found,iin=strlen(in);
for (i=j=0; i<iin; i++) {
  found=0;
  if (in[i]=='&') {
    for (k=i+1; k<iin; k++) if (in[k]==';') break;
    if (k<iin) {
      /* we found ';' we need to check if the entity 
       * is correct and then translate it */ 
      IFE(found=OgHtmlToPagecode1(k-(i+1),in+i+1,&code));
      if (found) {
        OgHtmlToPagecode2(in+i,k-i+1,out,&j,code,flag); i=k;
        }
      }
    }
  if (!found) {
    if (flag) out[j++]=in[i]; 
    else { out[j++]=0; out[j++]=in[i]; }
    }
  }

*iout=j;
if (flag) out[j++]=0; 
else { out[j++]=0; out[j++]=0; }
...
static int OgHtmlToPagecode1(int ientity, unsigned char *entity, int *pcode)
{
unsigned char *nil,e[DPcPathSize];
int found=0,i,len;

if (entity[0]=='#') {
  if (entity[1]=='x' || entity[1]=='X') {
    if (ientity-2<=4) {
      memcpy(e,entity+2,ientity-2); e[ientity-2]=0;
      *pcode=strtol(e,&nil,16); found=1;
      }
    }
  else {
    if (ientity-1<=4) {
      memcpy(e,entity+1,ientity-1); e[ientity-1]=0;
      *pcode=strtol(e,&nil,10); found=1;
      }
    }
  }
else {
  for (i=0; HtmlToUnicode[i].len!=0; i++) {
    len=HtmlToUnicode[i].len;
    if (len == ientity) {
     if (!memcmp(HtmlToUnicode[i].tag,entity,len)) {
       *pcode=HtmlToUnicode[i].code; found=1;  
       }
     }
    }
  }
...
static int OgHtmlToPagecode2(unsigned char *in, int iin, unsigned char *out, int *piout, int code, int flag)
{
int i,iout = *piout;
switch (flag) {
  case 0:
    out[iout++]=((code>>8)&0xff);
    out[iout++]=(code&0xff);
    break;
  case 1:
    if (code>=256) {
...
PUBLIC(int) OgHtmlToPagecodeTest(void)
{
int i;
for (i=0; HtmlToUnicode[i].len!=0; i++) {
  if (strlen(HtmlToUnicode[i].tag) != (size_t)HtmlToUnicode[i].len) {
    printf("%s: wrong length %d, should be %d",HtmlToUnicode[i].tag,HtmlToUnicode[i].len,strlen(HtmlToUnicode[i].tag));
    }
  }
DONE;
}
