...
for (i=0; i<ctrl_nls->icwd; i++) if (ctrl_nls->cwd[i]=='\\') ctrl_nls->cwd[i]='/';
...
if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
  OgMsg(ctrl_nls->hmsg,"gethostname",DOgMsgDestInLog
    ,"OgNlsInit: hostname is '%s'", ctrl_nls->hostname);
  }
