...
static og_bool setTimeout(struct og_ctrl_nls *ctrl_nls, char *conf, og_bool init, struct timeout_conf_context *ctx, int *timeout);
...
if (!OgFileExists(conf)) {
  if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
    OgMsg(ctrl_nls->hmsg,"noconf",DOgMsgDestInLog
      , "Configuration file '%s' does not exist", conf);
    }
  DONE;
  }
...
if (init) {
  if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
    OgMsg(ctrl_nls->hmsg,"readconf",DOgMsgDestInLog
      , "Reading configuration file '%s'", conf);
    }
  }
...
if (init) {
  ...
  if (found) {
    OgTrimString(value,value); ctrl_nls->conf->max_listening_threads=OgArgSize(value);
    if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
      OgMsg(ctrl_nls->hmsg,"max_listening_threads",DOgMsgDestInLog
        , "max_listening_threads: %d", ctrl_nls->conf->max_listening_threads);
      }
    }
...
if (init) {
  ...
  if (found) {
    ...
    if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
      OgMsg(ctrl_nls->hmsg,"permanent_threads",DOgMsgDestInLog
        , "permanent_threads: %d", ctrl_nls->conf->permanent_threads);
      }
    }
...
if (found) { OgTrimString(value,value); new_conf->backlog_max_pending_requests=OgArgSize(value); }
...
if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
  if (init) {
    OgMsg(ctrl_nls->hmsg,"backlog_max_pending_requests",DOgMsgDestInLog
      , "backlog_max_pending_requests: %d seconds", ctrl_nls->conf->backlog_max_pending_requests);
    }
  else if (new_conf->backlog_max_pending_requests != old_conf->backlog_max_pending_requests) {
    OgMsg(ctrl_nls->hmsg,"backlog_max_pending_requests",DOgMsgDestInLog
      , "backlog_max_pending_requests changed from %d to %d seconds"
      , old_conf->backlog_max_pending_requests, ctrl_nls->conf->backlog_max_pending_requests);
    }
  }
  ...
  og_bool backlog_timeout_specified = setTimeout(ctrl_nls, conf, init, timeout_ctx,
      &ctrl_nls->conf->backlog_timeout);
  ...
  if (answer_timeout_specified)
  {
    if (backlog_timeout_specified || socket_read_timeout_specified || request_processing_timeout_specified)
    {
      ...
      {
        sprintf(error_buffer, "NlsReadConfigurationFile: backlog_timeout_specified in conf file has been set to value %d while "
      ...
      {
        sprintf(error_buffer, "NlsReadConfigurationFile: socket_read_timeout in conf file has been set to value %d while "
      ...
      {
        sprintf(error_buffer, "NlsReadConfigurationFile: request_processing_timeout in conf file has been set to value %d while "
  ...
  og_bool backlog_indexing_timeout_specified = setTimeout(ctrl_nls, conf, init, timeout_ctx, &ctrl_nls->conf->backlog_indexing_timeout);
...
if (found) { OgTrimString(value,value); new_conf->loop_answer_timeout=OgArgSize(value); }
...
if (ctrl_nls->loginfo->trace & DOgNlsTraceMinimal) {
  if (init) {
    OgMsg(ctrl_nls->hmsg,"loop_answer_timeout",DOgMsgDestInLog
      , "loop_answer_timeout: %d milli-seconds", ctrl_nls->conf->loop_answer_timeout);
    }
  else if (new_conf->loop_answer_timeout != old_conf->loop_answer_timeout) {
    OgMsg(ctrl_nls->hmsg,"loop_answer_timeout",DOgMsgDestInLog
      , "loop_answer_timeout changed from %d to %d milli-seconds"
      , old_conf->loop_answer_timeout, ctrl_nls->conf->loop_answer_timeout);
    }
  }
