...
static int RqpReparenthesizeSubtree(struct og_ctrl_rqp *ctrl_rqp, int Inode, int *pstring_length, unsigned char **pstring)
{
int iuni_subtree_id; unsigned char uni_subtree_id[DPcPathSize*2];
...
int sequiv_length; unsigned char *sequiv;
int snode_length; unsigned char *snode;
int sson_length; unsigned char *sson;
...
if (node->is_subtree_root) {
  ...
  if (!found) {
    sprintf(erreur,"RqpReparenthesizeSubtree: could not find subtree_id from subtree_number=%d",node->subtree_number);
    OgErr(ctrl_rqp->herr,erreur); DPcErr;
    }
  IFE(OgCpToUni(strlen(subtree_id), subtree_id, DPcPathSize, &iuni_subtree_id, uni_subtree_id, DOgCodePageUTF8, 0, 0));
  ibuffer=0;
  memcpy(buffer+ibuffer,ctrl_rqp->subtree_function_name,ctrl_rqp->subtree_function_name_length); ibuffer+=ctrl_rqp->subtree_function_name_length;
  memcpy(buffer+ibuffer,"\0(",2); ibuffer+=2;
  memcpy(buffer+ibuffer,uni_subtree_id,iuni_subtree_id); ibuffer+=iuni_subtree_id;
  memcpy(buffer+ibuffer,"\0,\0 ",4); ibuffer+=4;
...
struct node *node=ctrl_rqp->Node+Inode;
int i,ilabel,length,truncated=0;
