...
{
int uni_tree_id_length; unsigned char uni_tree_id[DPcPathSize];
  ...
  switch (state) {
    ...
    case 1:
      if (c=='(') depth++;
        ...
        if (tree_depth>0 && tree_min_depth[tree_depth-1].depth==depth) {
          if (tree_depth>1) { subtree_number=tree_min_depth[tree_depth-2].subtree_number; tree_depth--; }
          else { subtree_number=(-1); tree_depth=0; }
          }
        depth--;
        }
      else if (cbefore!='\\' && c=='"') { if (in_dquote) { in_dquote=0; depth--; } else { in_dquote=1; start_dquote=1; depth++; } }
      else if (!PcIsspace(c)) {
        start=i; state=2;
        }
      break;
    ...
    case 2:
      ...
      if (c=='(') {
        ...
        if (i-start==ctrl_rqp->subtree_function_name_length && !Ogmemicmp(ctrl_rqp->subtree_function_name,s+start,ctrl_rqp->subtree_function_name_length)) {
          for (coma=0,in_quotes=0,j=i+2; j<is; j++) {
            cj=(s[j]<<8)+s[j+1]; cjm1=(s[j-2]<<8)+s[j-1]; 
            if (cjm1!='\\' && cj=='"') { if (in_quotes) in_quotes=0; else in_quotes=1; }            
            if (cj==',') { if (!in_quotes) { coma=1; break; } }
            }
          if (!coma) {
            sprintf(erreur,"RqpFirstParse: tree function does not have a marker followed by coma");
            OgErr(ctrl_rqp->herr,erreur); DPcErr;
            }  
          uni_tree_id_length=j-(i+2); memcpy(uni_tree_id,s+(i+2),uni_tree_id_length);
          ...
          OgTrimUnicode(uni_tree_id_length,uni_tree_id,&uni_tree_id_length,uni_tree_id);
          i=j; depth++; /** same operation as if in state 1 to erase the tree function **/
          current_subtree_number++; subtree_number=current_subtree_number;           
      ...
      else if (cbefore!='\\' && c=='"') { if (in_dquote) { in_dquote=0; depth--; } else { in_dquote=1; depth++; } }
      break;
    /** in functions which can have sub-functions, thus parenthesis levels **/
    case 3:
      if (c==')') { if (!in_func_dquote) parenthesis_level--; }
      else if (c=='(') { if (!in_func_dquote) parenthesis_level++; }
...
if (ctrl_rqp->loginfo->trace & DOgRqpTraceFirstParse) {
  OgMsg(ctrl_rqp->hmsg,"",DOgMsgDestInLog
    , "RqpFirstParse: after basic processing");
...
if (ctrl_rqp->loginfo->trace & DOgRqpTraceFirstParse) {
  OgMsg(ctrl_rqp->hmsg,"",DOgMsgDestInLog
    , "RqpFirstParse: after first RqpCleanMultipleOperator");
  IFE(OgRqpLogTree(ctrl_rqp));
  }
...
if (ctrl_rqp->loginfo->trace & DOgRqpTraceFirstParse) {
  OgMsg(ctrl_rqp->hmsg,"",DOgMsgDestInLog
    , "RqpFirstParse: after RqpSwapExceptOperator");
  IFE(OgRqpLogTree(ctrl_rqp));
  }
...
if (ctrl_rqp->loginfo->trace & DOgRqpTraceFirstParse) {
  OgThrMessageLog(ctrl_rqp->hmutex, DOgMlogInLog, ctrl_rqp->loginfo->where, 0
    , "RqpFirstParse: after NormalizeDepth");
  IFE(OgRqpLogTree(ctrl_rqp));
  }
