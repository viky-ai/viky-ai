...
struct og_ltrap_param cltrap_param, *ltrap_param=&cltrap_param;
...
IFn(ctrl_cut=(struct og_ctrl_cut *)malloc(sizeof(struct og_ctrl_cut))) {
...
IFn(ctrl_cut->hmsg=OgMsgInit(msg_param)) return(0);
IF(OgMsgTuneInherit(ctrl_cut->hmsg,param->hmsg)) return(0);
...
if (param->hmodule_to_inherit) {
  ...
  IFn(ctrl_cut->ha_cut=OgAutInit(aut_param)) return(0);
...
if (ctrl_cut->loginfo->trace & DOgLtrasTraceModuleCut) {
  ltrap_param->loginfo.trace |= DOgLtrapTraceAdd
                              + DOgLtrapTraceBuild
                              + DOgLtrapTraceSort
                              + DOgLtrapTraceSend;
  }
...
IFn(ltrap_param->ha_base=OgLtrasHaBase(ctrl_cut->hltras)) return(0);
IFn(ltrap_param->ha_swap=OgLtrasHaSwap(ctrl_cut->hltras)) return(0);
IFn(ltrap_param->ha_phon=OgLtrasHaPhon(ctrl_cut->hltras)) return(0);
...
IFn(ctrl_cut->hltrap=OgLtrapInit(ltrap_param)) return(0);
