...
struct og_ltras_xml_tag LtrasXmlReadTag[] =  {
  { DOgLtrasModuleReadConfTag_ltras_modules,          13, "ltras_modules" },
  { DOgLtrasModuleReadConfTag_ltras_module,           12, "ltras_module" },
  { DOgLtrasModuleReadConfTag_name,                   4, "name" },
  { DOgLtrasModuleReadConfTag_function_name,          13, "function_name" },
  { DOgLtrasModuleReadConfTag_library_name,           12, "library_name" },
  { DOgLtrasModuleReadConfTag_minimum_score,          13, "minimum_score" },
  ...
  { DOgLtrasModuleReadConfTag_parameters,             10, "parameters" },
  { DOgLtrasModuleReadConfTag_parameter,              9, "parameter" },
  { DOgLtrasModuleReadConfTag_equivalent_letter,      17, "equivalent_letter" },
  { DOgLtrasModuleReadConfTag_letter,                 6, "letter" },
  { 0, 0, "" }
  };
...
static og_status initTransformationAndScores(struct og_ctrl_ltras *ctrl_ltras, int some_maximum_transformation, int default_maximum_transformations);
...
static og_status ComputeEquivalentLetterTagContent(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info,
    unsigned char *buffer, int ibuffer, int value);
static og_status readTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib, int *value);
static og_status ComputeLtrasModuleTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info);
static og_status ComputeMinimumScoreTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib);
static og_status ComputeMinimumFinalScoreTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib);
static og_status ComputeMaximumTransformationTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib);
static og_status ComputeParameterTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib);
static og_status parseXmlContent(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib);
...
static og_status ComputeMaximumTransformationTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *b, int ib)
{
  ...
  if (0
      <= info->max_trans_nb_request_words && info->max_trans_nb_request_words < DOgLtrasMaximumTransformationLength)
  {
...
static og_status ComputeEquivalentLetterTag(struct og_ctrl_ltras *ctrl_ltras, struct og_ltras_xml_info *info, unsigned char *buffer, int ibuffer)
{
  if (!info->end_tag)
  {
    ...
    unsigned char *buffer, int ibuffer, int value)
{
      ...
      unsigned char *buffer, int ibuffer)
  {
    ...
    IFE(OgXmlDecode(ibuffer, buffer, DPcPathSize, &xml_decoded_buffer_length, xml_decoded_buffer,
        DOgCodePageUTF8, 0, 0));

    // Transformation of utf-8 xml decoded character to unicode
    unsigned char unibuffer[DPcPathSize];
    int unibuffer_length;
    IFE(OgCpToUni(xml_decoded_buffer_length, xml_decoded_buffer, DPcPathSize, &unibuffer_length, unibuffer,
        DOgCodePageUTF8, 0, 0));
...
for (i=imin+1; i<=imax; i++) {
  IFn(ms[i]) {
    for (j=i+1; j<=imax; j++) {
      IFx(ms[j]) { inext=j; break; }
      }
    a = ms[inext] - ms[ilast]; a /= inext -ilast;
    b = ms[ilast] - a*ilast;
    ms[i] = a*i + b;
    }
  else ilast=i;
...
for (i=imin+1; i<=imax; i++) {
  IFn(mt[i]) {
    for (j=i+1; j<=imax; j++) {
      IFx(mt[j]) { inext=j; break; }
      }
    a = mt[inext] - mt[ilast]; a /= inext -ilast;
    b = mt[ilast] - a*ilast;
    mt[i] = a*i + b;
    }
  else ilast=i;
