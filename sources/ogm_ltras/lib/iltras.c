...
static og_status LtrasXsdValidate(struct og_ctrl_ltras *ctrl_ltras);
...
struct og_lip_param clip_param,*lip_param=&clip_param;
struct og_rqp_param crqp_param,*rqp_param=&crqp_param;
struct og_aut_param caut_param,*aut_param=&caut_param;
struct og_pho_param cpho_param,*pho_param=&cpho_param;
struct og_ldi_param cldi_param,*ldi_param=&cldi_param;
struct og_stm_param cstm_param,*stm_param=&cstm_param;
...
size = ctrl_ltras->NodeNumber*sizeof(struct node);
...
struct og_ltra_module_param cmodule_param,*module_param=&cmodule_param;
char erreur[DOgErrorSize];
struct module *module;
int i,size;

ctrl_ltras->ModuleNumber = DOgLtrasModuleNumber;
size = ctrl_ltras->ModuleNumber*sizeof(struct module);
...
/** Test module, we keep it to have an example **/
//IFE(Imodule=LtrasAllocModule(ctrl_ltras,&module));
//module->start_name = ctrl_ltras->BaUsed;
//IFE(LtrasAppendBa(ctrl_ltras,4,"test"));
//ctrl_ltras->Ba[ctrl_ltras->BaUsed++]=0;
//module->length_name=4;
//module->init = OgLtrasModuleTestInit;
//module->module = OgLtrasModuleTest;
//module->flush = OgLtrasModuleTestFlush;

/** Add other modules from XML configuration file **/
IFE(LtrasXsdValidate(ctrl_ltras));
...
for (i=0; i<ctrl_ltras->ModuleUsed; i++) {
  ...
  IFn(module->handle=module->init(module_param)) {
    ...
    OgMsgErr(ctrl_ltras->hmsg,module_name,0,0,0,DOgMsgSeverityWarning,DOgErrLogFlagNotInErr+DOgErrLogFlagNoSystemError+DOgErrLogFlagNoDate);
    module->init=0;
    module->module=0;
    module->flush=0;
    }
...
static og_status LtrasXsdValidate(struct og_ctrl_ltras *ctrl_ltras)
{
  ...
  char *xml_file = ctrl_ltras->configuration_file;
  unsigned char xsd_file[DPcPathSize];
  if (ctrl_ltras->WorkingDirectory[0])
  {
    sprintf(xsd_file, "%s/conf/xsd/ltras_conf.xsd", ctrl_ltras->WorkingDirectory);
  }
  else
  {
    sprintf(xsd_file, "conf/xsd/ltras_conf.xsd");
  }
  ...
  if (OgFileExists(xml_file))
  {
    ...
    {
      int good_format;
      IFE(good_format = OgXmlCheckXsd(ctrl_ltras->hmsg, xsd_file, xml_file));
      if (!good_format)
      {
        unsigned char erreur[DPcPathSize];
        sprintf(erreur, "LtrasXsdValidate: file '%s' is not validated by '%s'", xml_file, xsd_file);
    ...
    {
      OgMsg(ctrl_ltras->hmsg, "", DOgMsgDestInLog,
          "LtrasXsdValidate: warning: file '%s' does not exist, could not check '%s'", xsd_file, xml_file);
    }
  }
  DONE;
}
