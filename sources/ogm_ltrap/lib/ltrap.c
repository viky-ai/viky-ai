...
static int OgLtrapAddSpan(struct og_ctrl_ltrap *ctrl_ltrap,int lexicon_type,int iuni,unsigned char *uni,int icurrent);
...
for (i=start; i<iuni; i+=2) {
  ...
  if (OgUniIspunct(c)) {
    ...
    IFE(overlapped=OgLtrapAddSpan(ctrl_ltrap,input->lexicon_type,iuni,uni,i));
    if (!overlapped) {
...
static int OgLtrapAddSpan(struct og_ctrl_ltrap *ctrl_ltrap,int lexicon_type,int iuni,unsigned char *uni,int icurrent)
{
int min_header_length,max_header_length,header_length,unicode_header_length;
int attribute_number,language,position,frequency;
struct og_ltrap_input *input = ctrl_ltrap->input;
int ibuffer; unsigned char *p,buffer[DPcPathSize];
unsigned char out[DPcAutMaxBufferSize+9];
int i,cuni,cout,length,overlapped=0,end;
...
max_header_length = ctrl_ltrap->max_header_length;
...
for (header_length=min_header_length; header_length<max_header_length; header_length++) {
  unicode_header_length = header_length*2;
  if (icurrent+unicode_header_length > iuni) break;
  ibuffer=0; for (i=0; i<unicode_header_length; i++) buffer[ibuffer++]=uni[icurrent+i];
...
unicode_header_length = max_header_length*2;
...
if (icurrent+unicode_header_length <= iuni) {
  ...
  if ((retour=OgAufScanf(ha_lexicon,ibuffer,buffer,&iout,out,&nstate0,&nstate1,states))) {
    do {
      IFE(retour);
      for (i=0; i<iout; i+=2) {
        if (icurrent+unicode_header_length+i == iuni) { end=1; cuni = 0; }
        else { end=0; cuni = (uni[icurrent+unicode_header_length+i]<<8) + uni[icurrent+unicode_header_length+i+1]; }
        cout = (out[i]<<8) + out[i+1];
        if (cout == 0x1) {
          ...
          IFE(DOgPnin4(ctrl_ltrap->herr,&p,&frequency));
          length = i+unicode_header_length;
          //if (lexicon_type == DOgLtrapLexiconTypeSwap) { 
          if (iout > p-out) {
            iword = iout-(p-out); word=p; 
            IFE(LtrapSpanAdd(ctrl_ltrap,icurrent,length,iword,word,frequency));
