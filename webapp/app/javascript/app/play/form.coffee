class PlayForm
  ...
  dispatch: (event) ->
    ...
    if action == 'play-switch-agent'
      ...
      disable_with = $('.play-form button').data('disable-with')
      $('.play-form button').removeAttr('data-disable-with')
      $('#play_input_ownername').val(agent.data('ownername'));
      $('#play_input_agentname').val(agent.data('agentname'));
      Rails.fire($('.play-form')[0], 'submit')
      $('.play-form button').attr('data-disable-with', disable_with)
