...
static og_status NlpMatchExpressions(og_nlp_th ctrl_nlp_th)
{
  ...
  for (size_t i = 0; i < request_input_part_used; i++)
  {
    ...
    OgMsg(ctrl_nlp_th->hmsg, "", DOgMsgDestInLog, "found expression '%s' with input_parts %d:%d", text,
        expression->input_part_start, expression->input_parts_nb);
...
static og_status NlpMatchInterpretation(og_nlp_th ctrl_nlp_th, struct interpretation *interpretation)
{
  ...
  for (int i = 0; i < interpret_package_used; i++)
  {
    ...
    IFE(input_part_added = NlpMatchInterpretationInPackage(ctrl_nlp_th, interpretation, interpretation_id, i, interpret_package->package));
    ...
    og_string interpretation_id, int Iinterpret_package, package_t package)
{
  ...
  og_string package_slug = OgHeapGetCell(package->hba, package->slug_start);
  IFN(package_slug) DPcErr;
  OgMsg(ctrl_nlp_th->hmsg, "", DOgMsgDestInLog, "Found input part %d in package '%s' '%s':", Iinput_part, package_slug,
      package_id);
