...
og_status NlpRequestExpressionLog(og_nlp_th ctrl_nlp_th, struct request_expression *request_expression, int offset)
{
  ...
  char language[DPcPathSize];
  language[0] = 0;
  if (request_expression->language->id != DOgLangNil) snprintf(language, DPcPathSize, " language=%s:%.2f",
      OgIso639ToCode(request_expression->language->id), request_expression->language->score);
