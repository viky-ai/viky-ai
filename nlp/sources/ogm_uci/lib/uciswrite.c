...
PUBLIC(int) OgUciServerWrite(void *handle, struct og_ucisw_input *input)
{
...
if (input->http_status <= 0)
{
  input->http_status = 200;
  input->http_status_message = "OK";
}
...
sprintf(header,
  ...
  "Content-length: %d\r\n"
  "\r\n"
  , input->http_status, input->http_status_message
