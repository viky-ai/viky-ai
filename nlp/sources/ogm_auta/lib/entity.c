...
/* C0 Controls and Basic Latin */
  ...
  {"apos"    ,39}, /* apostrophe, U+0027 ISOnum */
...
PUBLIC(void *) OgEntityInit(param)
struct og_entity_param *param;
{
char erreur[DOgErrorSize];
struct og_ctrl_entity *ctrl_entity;
struct og_aut_param caut_param,*aut_param=&caut_param;
char buffer[DPcPathSize],entity[DPcPathSize];
int i;

IFn(ctrl_entity=(struct og_ctrl_entity *)malloc(sizeof(struct og_ctrl_entity))) {
  sprintf(erreur,"OgEntityInit: malloc error on ctrl_entity");
  OgErr(param->herr,erreur); return(0);
  }
memset(ctrl_entity,0,sizeof(struct og_ctrl_entity));

ctrl_entity->herr = param->herr;
ctrl_entity->hmutex = param->hmutex;
ctrl_entity->cloginfo = param->loginfo;
...
for (i=0; OgEntity[i].value!=0; i++) {
  strcpy(entity,OgEntity[i].name); //PcStrlwr(entity);
  sprintf(buffer,"c:%s:%d",entity,OgEntity[i].value);
  IF(OgAutAdd(ctrl_entity->ha,strlen(buffer),buffer)) return(0);
  sprintf(buffer,"n:%d:%s",OgEntity[i].value,entity);
  IF(OgAutAdd(ctrl_entity->ha,strlen(buffer),buffer)) return(0);
  }

return((void *)ctrl_entity);
}




PUBLIC(int) OgEntityFlush(handle)
void *handle;
{
struct og_ctrl_entity *ctrl_entity = (struct og_ctrl_entity *)handle;
IFE(OgAutFlush(ctrl_entity->ha));
DPcFree(ctrl_entity);
DONE;
}
...
PUBLIC(int) OgEntityGetCode(handle,entity,pcode)
void *handle; char *entity; int *pcode;
{
struct og_ctrl_entity *ctrl_entity = (struct og_ctrl_entity *)handle;
...
if ((retour=OgAutScanf(ctrl_entity->ha,-1,buffer,&iout,out,&nstate0,&nstate1,states))) {
  ...
  while((retour=OgAutScann(ctrl_entity->ha,&iout,out,nstate0,&nstate1,states)));
  }
return(0);
}
