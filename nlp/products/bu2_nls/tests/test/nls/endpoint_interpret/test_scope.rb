  ...
  module EndpointInterpret
    ...
    class TestScope < NlsTestCommon
      ...
      def create_package(id)
        ...
        interpretation_public << Expression.new("public scope#{id}", glue_distance: 0)

        interpretation_private = package.new_interpretation("scope_private_#{id}", { scope: "private" })
        interpretation_private << Expression.new("private scope#{id}", glue_distance: 0)

        interpretation_hidden = package.new_interpretation("scope_hidden_#{id}", { scope: "hidden" })
        interpretation_hidden << Expression.new("hidden scope#{id}", glue_distance: 0)

        package
      end

      def test_scope
        check_interpret("public scope1, public scope2, private scope1, private scope2, hidden scope1, hidden scope2",
